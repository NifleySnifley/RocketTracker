...
private:
	...
	gpio_num_t cs, rst;
	gpio_num_t dio0, sck, stx, srx;
	bool use_dma = false;

	spi_device_handle_t spi_inst;
...
public:
	...
	RFM97_LoRa(spi_host_device_t SPI, gpio_num_t CS, gpio_num_t DIO0, gpio_num_t RST, gpio_num_t STX, gpio_num_t SRX, gpio_num_t SCK, bool use_dma);
	...
	void HAL_spiwrite(uint8_t reg, uint8_t value);
	...
	void HAL_spiwrite(uint8_t start, uint8_t* dataptr, size_t len);

	/// Reads from register at address reg
	/// @param reg register address to read from
	/// @return value stored at address
	uint8_t HAL_spiread(uint8_t reg);
	...
	void HAL_spiread(uint8_t start, uint8_t* dataout, size_t len);
...
#if HAL_ENABLE_DMA
	bool HAL_writeFIFO_DMA(uint8_t* bytes, size_t n);
#endif
...
#if HAL_ENABLE_DMA
	bool HAL_readFIFO_DMA(volatile uint8_t* bytes, size_t n);
#endif

	// Make sure all of the FIFO pointers are correct
	void initFIFO();

	bool HAL_spiInit(uint baud);
