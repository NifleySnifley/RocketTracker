...
void LogDisplay::AddLog(const char* fmt, ...) IM_FMTARGS(2) {
...
void LogDisplay::Draw(const char* title, bool* p_open) {
...
void DebugViewer::init() {
	this->log.Clear();
	this->link->rx_signal.connect(boost::bind(&DebugViewer::datum_callback, this, _1, _2));
}
...
void DebugViewer::render_gui(bool* open) {
	...
	if (*open) {
		...
		this->log.Draw("Debug Viewer", open);
	}
}
